...
function displayBookedActivities(data, remType, showMore) {
	...
	for(; index < bookedActivities.length; index++) {
		...
		if(remType == "lessons") {
			title = "Lezione di " + bookedActivities[index].course.name + " di " + weekday[date.getDay()] + " " + date.getDate() + " " + months[date.getMonth()] + " " + date.getFullYear();
		}
		else if(remType == "seminars") {
			title = "Seminario di " + weekday[date.getDay()] + " " + date.getDate() + " " + months[date.getMonth()] + " " + date.getFullYear();
		}
